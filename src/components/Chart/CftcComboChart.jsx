// src/components/Chart/CftcComboChart.jsx
// Renders a Recharts ComposedChart with:
//   - Line (left Y-axis) → absolute weekly value
...
// ── Main Component ────────────────────────────────────────────
export default function CftcComboChart({ rawData, metricId, label }) {
    const item = ITEM_MAP[metricId];

    const chartData = useMemo(() => {
        if (!rawData || rawData.length === 0 || !metricId) return [];

        // Filter and sort rows from 2026-01-01 onwards
        ...
        return filtered.map(({ row, datePart }, i) => {
            ...
            if (item?.isSeriesComputed) {
                // WoW metric: compute absolute of base column
            ...
            // Weekly change = current absolute - previous absolute
    ...
    if (chartData.length === 0) {
        return (
            <div style={{
                ...
                <div>No data available from 2026-01-01 onwards.</div>
            ...
            <div style={{ width: '100%', height: 340 }}>
                <ResponsiveContainer width="100%" height="100%">
                    ...
                    >
                        ...
                        {/* LINE on top (renders after bars) */}
                        <Line
                            yAxisId="left"
                            type="monotone"
                            dataKey="absolute"
