// src/components/Input/CftcQuickSelector.jsx
//
// Toolbar component for multiline CFTC tabs.
...
export default function CftcQuickSelector({ tab }) {
    ...
    const handleSlotChange = async (slotIndex, { id: metricId, label }) => {
        ...
        try {
            ...
            dispatch(setTabMultiChart({ tabId: tab.id, series: cleanSeries, product }));
        } catch (err) {
            console.error('[CftcQuickSelector] slot', slotIndex, 'error:', err);
        } finally {
            setLoadingSlot(null);
        }
    };
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flex: 1, minWidth: 0 }}>
            {slots.map((slot, i) => {
                ...
                return (
                    ...
                    >
                        {/* Slot label */}
                        <div style={{ fontSize: '10px', color: '#4a6a8a', fontWeight: 600, marginBottom: '2px', letterSpacing: '0.05em' }}>
                            COL {i + 1}
                        </div>
                        ...
                        {isLoadingThis && (
                            <div style={{
                                ...
                                <span style={{ fontSize: '10px', color: '#26a69a', fontWeight: 600 }}>
                                    Loading…
                                </span>
                            </div>
                        )}
