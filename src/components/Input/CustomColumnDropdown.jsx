...
export default function CustomColumnDropdown({ value, onChange, disabled }) {
    ...
    const renderItem = (item, fullLabel, style) => (
        <div
            key={item.id}
            style={style(item.id === value)}
            onClick={() => handleSelect(item, fullLabel)}
            onMouseEnter={(e) => { if (item.id !== value) e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }}
