...
async function deleteNon2025Data(tableName) {
...
async function processFile(filePath, tableConfig) {
    ...
    console.log(`Processing ${fileName} for ${tableName}...`);
        ...
        fs.createReadStream(filePath)
            ...
            .on('data', (row) => {
                ...
                if (normalizedRow[marketKey] && normalizedRow[marketKey].trim() === marketName) {
                    ...
                    if (dateStr && dateStr.startsWith('2025')) return;
    ...
    for (const update of updates) {
        ...
        try {
            ...
            // Log real errors other than unique constraint violations if we cared, 
            // but since we deleted everything non-2025, any error is real.
            // Wait, we removed ON CONFLICT? Yes.
            if (insertCount < 3) console.error(`Insert error: ${e.message}`);
        }
    }
...
async function main() {
    try {
        ...
        for (const tableConfig of TABLES_TO_PROCESS) {
            ...
            await deleteNon2025Data(tableConfig.tableName);

            // 2. Import all other years
            let totalForTable = 0;
            for (const file of files) {
                const count = await processFile(path.join(__dirname, file), tableConfig);
                totalForTable += count;
            }
            console.log(`Total new rows for ${tableConfig.tableName}: ${totalForTable}`);
        }
